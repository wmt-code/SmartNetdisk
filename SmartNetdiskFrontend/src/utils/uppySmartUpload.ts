...
export class SmartUploadPlugin extends BasePlugin<SmartUploadPluginOptions, any, any> {
  ...
  // 文件夹路径缓存（类级别，跨批次复用）
  private folderCache = new Map<string, Promise<number>>()
  ...
  private preprocessor = async (fileIDs: string[]) => {
    ...
    // 使用类级别的 folderCache（跨批次复用）
    const folderCache = this.folderCache
    ...
    const queue = [...fileIDs]
    const workers = Array(Math.min(fileIDs.length, CONCURRENT_LIMIT))
      .fill(null)
      .map(async () => {
  ...
  private async uploadDirectly(file: UppyFile<any, any>, fileData: File) {
    ...
    const result = await uploadFile(fileData, meta.folderId ?? this.folderId, (percent) => {
      ...
      console.log(
        `[SmartUpload] 普通上传进度: ${percent}% (${bytesUploaded}/${fileData.size} bytes)`,
      )
  ...
  private async uploadWithChunks(file: UppyFile<any, any>, fileData: File) {
    ...
    console.log(
      ...
      '进度:',
      ((initialUploadedBytes / fileData.size) * 100).toFixed(2) + '%',
    )
    ...
    const updateProgress = (phase: 'upload' | 'merge' = 'upload', mergeProgress = 0) => {
      ...
      console.log(
        `[SmartUpload] 进度更新 [${phase}]:`,
        (progressPercent * 100).toFixed(2) + '%',
        `(${uploadedBytes}/${fileData.size} bytes)`,
      )
    ...
    for (let i = 0; i < pendingChunks.length; i += CONCURRENT_LIMIT) {
      ...
      console.log(
        `[SmartUpload] 开始上传批次 ${Math.floor(i / CONCURRENT_LIMIT) + 1}, 分片索引:`,
        batch,
      )
      ...
      await Promise.all(
        batch.map(async (chunkIndex) => {
          ...
          console.log(
            `[SmartUpload] 分片 ${chunkIndex + 1}/${totalChunks} 开始上传, 大小:`,
            currentChunkSize,
          )
