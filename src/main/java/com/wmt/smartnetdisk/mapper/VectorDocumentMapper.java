...
public interface VectorDocumentMapper extends BaseMapper<VectorDocument> {
        ...
        /**
         * 插入向量文档（带 vector 类型转换）
         *
         * @param doc 向量文档
         * @return 影响行数
         */
        @Insert("""
                        INSERT INTO vector_document (file_id, user_id, chunk_index, content, embedding, token_count, create_time)
                        VALUES (#{fileId}, #{userId}, #{chunkIndex}, #{content}, #{embeddingStr}::vector, #{tokenCount}, NOW())
                        """)
        @Options(useGeneratedKeys = true, keyProperty = "id")
        int insertVectorDocument(VectorDocument doc);
}
